...
function initVirtualDevice(){
    ...
    device.addControl("Dimmer Range", { type: "range", value: 0, order: 5, max: 100 });
    device.addControl("Led State", { type: "switch", value: false, order: 6 });
    device.addControl("Led Hue", { type: "range", value: 0, order: 7, max: 360 });
    device.addControl("Led Saturation", { type: "range", value: 0, order: 8, max: 100 });
    device.addControl("Led Brightness", { type: "range", value: 0, order: 9, max: 100 });
    device.addControl("Scene1", { type: "switch", value: false, order: 10 });
    device.addControl("Scene2", { type: "switch", value: false, order: 11 });
    device.addControl("Heat Icon", { type: "range", value: 0, order: 12, max: 3 });
    device.addControl("House Icon", { type: "range", value: 0, order: 13, max: 3 });
    device.addControl("Motion Icon", { type: "range", value: 0, order: 14, max: 3 });
    ...
    device.addControl("Therm Room Temp", { type: "temperature", value: 0, order: 18 });
    device.addControl("Therm Set Temp", { type: "value", value: 0, order: 19, readonly: false });
    ...
    device.addControl("Initialized", { type: "switch", value: false, order: 24, readonly: true });
    device.addControl("Status", { type: "text", value: null, order: 25, readonly: false });
    device.addControl("Send Raw", { type: "text", value: null, order: 26, readonly: false });
    ...
    defineRule({
        ...
        then: function (newValue, devName, cellName) {
            switch (cellName) {
                ...
                case "Heat Icon":
                    if (newValue > 0)
                        sendCommand({ "warnings": [{ "id": 1, "type": wtype.heat, "state": newValue }] })
                    else
                        sendCommand({ "warnings": [{ "id": 1, "type": wtype.blank, "state": 0 }] })
                    break
                case "House Icon":
                    if (newValue > 0)
                        sendCommand({ "warnings": [{ "id": 2, "type": wtype.house, "state": newValue }] })
                    else
                        sendCommand({ "warnings": [{ "id": 2, "type": wtype.blank, "state": 0 }] })
                    break
                case "Motion Icon":
                    if (newValue > 0)
                        sendCommand({ "warnings": [{ "id": 3, "type": wtype.robot, "state": newValue }] })
                    else
                        sendCommand({ "warnings": [{ "id": 3, "type": wtype.blank, "state": 0 }] })
                    break
                case "Zap Icon":
                    if (newValue > 0)
                        sendCommand({ "warnings": [{ "id": 4, "type": wtype.zap, "state": newValue }] })
                    else
                        sendCommand({ "warnings": [{ "id": 4, "type": wtype.blank, "state": 0 }] })
                    break
                case "Therm State":
                    sendCommand({ "sync": { pid: p.Therm, "therm": { state: Number(newValue) } } })
                    break
                case "Therm Heat":
                    sendCommand({ "sync": { pid: p.Therm, "therm": { heat: Number(newValue) } } })
                    break
                case "Bulb":
                    sendCommand({ "sync": { pid: p.Main, "buttons": { bid: b.Bulb, state: Number(newValue) } } })
                    break
                case "Dimmer State":
                    sendCommand({ "sync": { pid: p.Main, "buttons": { bid: b.Dimmer, state: Number(newValue) } } })
                    break
                case "Led State":
                    sendCommand({ "sync": { pid: p.Main, "buttons": { bid: b.Led, state: Number(newValue) } } })
                    break
                case "Scene1":
                    sendCommand({ "sync": { pid: p.Main, "buttons": { bid: b.Scene1, state: Number(newValue) } } })
                    break
                case "Scene2":
                    sendCommand({ "sync": { pid: p.Main, "buttons": { bid: b.Scene2, state: Number(newValue) } } })
                    break
            }
        }
    });
...
function action(data) {
    switch (data["pid"]) {
        ...
        case p.Dimmer: //{"dimmer":{"pid":p.Dimmer,"power":1}}
            if ("power" in data)
                writeWbTopic(idVirtDev, "Dimmer State", Boolean(data["power"]))


            if ("dimmer" in data)
                writeWbTopic(idVirtDev, "Dimmer Range", Number(data["dimmer"]))
            break;
        case p.Led:

            if ("power" in data)
                writeWbTopic(idVirtDev, "Led State", Boolean(data["power"]))
        ...
        case p.Therm:
            if ("state" in data)
                writeWbTopic(idVirtDev, "Therm State", Boolean(data["state"]))

            if ("heat" in data)
                writeWbTopic(idVirtDev, "Therm Heat", Boolean(data["heat"]))

            if ("set" in data)
                writeWbTopic(idVirtDev, "Therm Set Temp", Number(data["set"]))
            break;
    }
}
...
function sendPage(pid, syncType) {
    ...
    switch (pid) {
        case p.Main:
            page = {
                pid: p.Main, name: "Main", format: 5, buttons: [
                    { "bid": b.Bulb, "label": "Bulb", "type": btype.toggle, "icon": bicns.bulb, "state": Number(dev[idVirtDev]["Bulb"]) },
                    { "bid": b.Dimmer, "label": "Dimmer", "type": btype.dimmer, "next": p.Dimmer, "icon": bicns.dimmer, "state": Number(dev[idVirtDev]["Dimmer State"]) },
                    { "bid": b.Led, "label": "Led", "type": btype.dimmer_color, "next": p.Led, "icon": bicns.dimmer_color, "state": Number(dev[idVirtDev]["Led State"]) },
                    { "bid": b.Therm, "label": "Therm", "type": btype.page, "next": p.Therm, "state": pf.therm, "icon": bicns.heat },
                    { "bid": b.Scene1, "label": "Scene1", "type": btype.toggle, "icon": bicns.bed, "state": Number(dev[idVirtDev]["Scene1"]) },
                    { "bid": b.Scene2, "label": "Scene2", "type": btype.toggle, "icon": bicns.music, "state": Number(dev[idVirtDev]["Scene2"]) },
                ]
            }
            break;
        case p.Dimmer:
            page = {
                "pid": p.Dimmer, "format": pf.dimmer, "name": "Dimmer", "min": 0, "max": 100, "icon": bicns.dimmer,
                "power": Number(dev[idVirtDev]["Dimmer State"]), "dimmer": dev[idVirtDev]["Dimmer Range"]
            }
            break;
        case p.Led:
            page = {
                "pid": p.Led, "format": pf.dimmer_color, "name": "Led", "power": Number(dev[idVirtDev]["Led State"]), "icon": bicns.dimmer_color,
                "hsbcolor": "{},{},{}".format(
        ...
        case p.Therm:
            page = {
                "pid": p.Therm, "format": pf.therm, "name": "Therm", "therm": {
    ...
    if (syncType == "refresh") {
        ...
        if (pid == p.Main) // a crutch to close the bug with status updates for the page under the swipe
        ...
        for (key in obj) {
            switch (key) {
                ...
                case "ANALOG":
                    ...
                    sendCommand({ "summary": { "title": "{} °C".format(newValue), "text": "Room" } })
                    break
                case "ESP32":
...
function initConst() {
    pf = {
        home: 1, buttons2: 2, buttons3: 3, buttons4: 4, buttons6: 5, buttons8: 6, dimmer: 7, dimmer_color: 8,
        therm: 9, alert1: 10, alert2: 11, alarm: 12, media: 13, playlist: 14, status: 15
    }

    btype = {
        unused: 0, toggle: 1, push: 2, dimmer: 3, dimmer_color: 4, page: 10
    }

    bicns = {
        blank: 0, bulb: 1, dimmer: 2, dimmer_color: 3, vacuum: 4, bed: 5, house: 6, sofa: 7,
        bell: 8, heat: 9, curtains: 10, music: 11, binary: 12, fan: 13, switch: 14, talk: 15, info: 16
    }

    wtype = {
        blank: 0, heat: 1, house: 2, light: 3, plug: 4, robot: 5, speaker: 6, zap: 7, bustbin: 8
    }

    wcolor = {
        none: 0, white: 1, red: 2, blue: 3
    }

    weather_icons = {
        clear_sky: "01d", few_clouds: "02d", scatted_clouds: "03d", broken_clouds: "04d",
        shower_rain: "09d", rain: "10d", thunderstorm: "11d", snow: "13d", mist: "50d"
    }
}
