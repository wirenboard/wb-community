...
function createVakioBaseSmart(params) {
  //Создаем функцию
  ...
  var hidden_update =
    params.hidden_update == undefined ? false : params.hidden_update; //Если параметр hidden_update объявлен, то передаем его в переменную hidden_update, в ином случае в переменную передаем "false"
  var hidden_reset =
    params.hidden_reset == undefined ? false : params.hidden_reset; //Если параметр hidden_reset объявлен, то передаем его в переменную hidden_reset, в ином случае в переменную передаем "false"
  ...
  getDevice(nameVirtualDevice).addControl(mqttTopicSeries, {
    type: "text",
    order: 3,
    readonly: true,
    value: "Loading",
  });
  getDevice(nameVirtualDevice).addControl(mqttTopicSubType, {
    type: "text",
    order: 4,
    readonly: true,
    value: "Loading",
  });
  getDevice(nameVirtualDevice).addControl(mqttTopicXtalFreq, {
    type: "text",
    order: 5,
    readonly: true,
    value: "Loading",
  });
  getDevice(nameVirtualDevice).addControl(mqttTopicMAC, {
    type: "text",
    order: 6,
    readonly: true,
    value: "Loading",
  });
  getDevice(nameVirtualDevice).addControl(mqttTopicFWversion, {
    type: "text",
    order: 7,
    readonly: true,
    value: "Loading",
  });
  getDevice(nameVirtualDevice).addControl(mqttTopicConnect, {
    type: "switch",
    order: 10,
    readonly: true,
    value: false,
  ...
  sendTranslate(
    mqttTopicSpeed,
    '{"type": "range","order":"2","max":"7","readonly":0,"title":{"ru": "Скорость"}}'
  );
  sendTranslate(
    mqttTopicSeries,
    '{"type": "text","order":"3","readonly": "1","title":{"ru": "Серия"}}'
  );
  sendTranslate(
    mqttTopicSubType,
    '{"type": "text","order":"4","readonly": "1","title":{"ru": "Тип вентилятора"}}'
  );
  sendTranslate(
    mqttTopicXtalFreq,
    '{"type": "text","order":"5","readonly": "1","title":{"ru": "Частота кварцевого генератора"}}'
  );
  sendTranslate(
    mqttTopicMAC,
    '{"type": "text","order":"6","readonly": "1","title":{"ru": "MAC адрес"}}'
  );
  sendTranslate(
    mqttTopicFWversion,
    '{"type": "text","order":"7","readonly": "1","title":{"ru": "Версия ПО"}}'
  );
  sendTranslate(
    mqttTopicConnect,
    '{"type": "switch","order":"10","readonly": "1","title":{"ru": "Соединение"}}'
  );
  ...
  if (!hidden_update) {
    getDevice(nameVirtualDevice).addControl(mqttTopicUpdate, {
      type: "pushbutton",
      value: false,
      order: 8,
    });
    sendTranslate(
      mqttTopicUpdate,
      '{"type": "pushbutton","order":"8","readonly": 0,"title":{"ru": "Обновить ПО"}}'
    );
    ...
    if (getDevice(nameVirtualDevice).isControlExists(mqttTopicUpdate)) {
      getDevice(nameVirtualDevice).removeControl(mqttTopicUpdate, {
        type: "pushbutton",
        value: false,
        order: 8,
      });
  ...
  if (!hidden_reset) {
    getDevice(nameVirtualDevice).addControl(mqttTopicReset, {
      type: "pushbutton",
      value: false,
      order: 9,
    });
    sendTranslate(
      mqttTopicReset,
      '{"type": "pushbutton","order":"9","readonly": 0,"title":{"ru": "Сбросить до заводских"}}'
    );
    ...
    if (getDevice(nameVirtualDevice).isControlExists(mqttTopicReset)) {
      getDevice(nameVirtualDevice).removeControl(mqttTopicReset, {
        type: "pushbutton",
        value: false,
        order: 9,
      });
  ...
  trackMqtt(
    "/devices/{}/controls/{}/on".format(nameVirtualDevice, mqttTopicWorkmode),
    function (message) {
  ...
  function errorTopic(nameVirtualDevice, nameControl, value) {
    value = value == true ? "r" : "";
    publish(
      "/devices/{}/controls/{}/meta/error".format(
        nameVirtualDevice,
        nameControl
      ),
      "{}".format(value),
  ...
  trackMqtt("{}/system".format(topic), function (message) {
    ...
    if (!message.value.indexOf("0601")) {
      var hardwareInfoObj = JSON.parse(
        message.value.slice(4, message.value.lenght)
      );
